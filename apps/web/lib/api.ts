...
async function safeFetch(input: RequestInfo | URL, init?: RequestInit) {
  try {
    ...
    throw new Error('Network request failed. Backend may be unreachable, blocked by CORS, or still waking up.');
  }
}
...
export async function persistSessionCookies(accessToken: string, refreshToken: string) {
  const res = await safeFetch('/api/auth/store', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    credentials: 'include',
    body: JSON.stringify({ accessToken, refreshToken }),
  });
  if (!res.ok) throw new Error(await parseResponseError(res, 'Failed to persist session cookies'));
}
...
export async function loginUser(email: string, password: string): Promise<AuthResponse> {
  ...
  return validateAuthResponse(await res.json(), 'Login succeeded but auth tokens were missing.');
}

export async function registerUser(full_name: string, email: string, password: string): Promise<AuthResponse> {
  ...
  return validateAuthResponse(await res.json(), 'Signup succeeded but auth tokens were missing.');
}
...
export async function apiFetch<T = any>(path: string, init: RequestInit = {}, retry = true): Promise<T> {
  ...
  if (res.status === 401 && retry && getRefreshToken()) {
    ...
    if (nextAccess) {
      ...
      res = await safeFetch(`${API_URL}${path}`, { ...init, headers });
    }
  }
...
export async function downloadProtectedFile(path: string, filename: string) {
  let token = getToken();
  if (!token && getRefreshToken()) token = await refreshAccessToken();
  if (!token) throw new Error('You are not logged in. Please sign in again.');

  let res = await safeFetch(`${API_URL}${path}`, {
    method: 'GET',
  ...
  if (res.status === 401 && getRefreshToken()) {
    const nextAccess = await refreshAccessToken();
    if (!nextAccess) throw new Error('Your session expired. Please sign in again.');
